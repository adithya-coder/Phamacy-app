...
public class UpdateMedicine extends AppCompatActivity {
    ...
    private TextInputEditText  medicineName, price_field, email_field, manufature_field, des_field;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (ContextCompat.checkSelfPermission(UpdateMedicine.this, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(UpdateMedicine.this, new String[]{
        ...
        price_field.setText(medicine.getMediPrice());
        email_field.setText(medicine.getEmail());
        manufature_field.setText(medicine.getManfacture());
        des_field.setText(medicine.getDescription());
        if (medicine.getImage() != null) {
            imageButton.setImageIcon(Icon.createWithBitmap(decodeBitmapAndSaveToFirebase(medicine.getImage())));
            setImage(medicine.getImage());
        }
        ...
        update_button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                String medicine1 = medicineName.getText().toString();
                ...
                if (TextUtils.isEmpty(medicine1) && TextUtils.isEmpty(price) && TextUtils.isEmpty(email) && TextUtils.isEmpty(manufacture)) {
                    ...
                    update(medicine1, price, email, manufacture, des, image);
                }
            }
        });
    }
     ...
     * @param image
     */
    private void update( String medicine1, String price, String email,String manufature, String des, String image) {
        Medicine stock = (Medicine) getIntent().getSerializableExtra("medicine");
        Medicine medicine = new Medicine();
        medicine.setMedicineName(medicine1);
        medicine.setMediPrice(price);
        medicine.setEmail(email);
        medicine.setManfacture(manufature);
        medicine.setDescription(des);
        medicine.setImage(image);
        medicine.setUserId(FirebaseAuth.getInstance().getCurrentUser().getUid());
        db.collection("stock").document(stock.getId()).set(medicine).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                ...
                TastyToast.makeText(
                        getApplicationContext(),
                        "Your Medicine has been Successfully update!",
                        TastyToast.LENGTH_LONG,
                        TastyToast.SUCCESS
